...
export default function Home() {
    ...
    // Notifications State
    const [notifications, setNotifications] = useState({ requests: 0, messages: 0, admin: 0 });
    ...
    // Fetch pending requests details when dropdown opens
    useEffect(() => {
            ...
            if (isFav) {
                ...
                if (res.ok) {
                    ...
                    if (data.message === 'jwt expired') {
                        addToast("انتهت صلاحية الجلسة، يرجى تسجيل الدخول مجدداً");
                        handleLogout();
                    } else {
        ...
        <div className="min-h-screen text-white font-['Tajawal'] selection:bg-purple-500/30 overflow-x-hidden relative" style={{ background: '#0a0a0a' }} dir="rtl">
            ...
            <nav className="fixed top-0 w-full z-50 backdrop-blur-md bg-[#0a0a0a]/80 border-b border-white/10 px-6 py-4 flex justify-between items-center">
                ...
                <div className="flex items-center gap-3 md:gap-5 text-white">
                    ...
                    {/* Requests Icon with Dropdown */}
                    <div className="relative">
                        <button
                            onClick={() => setIsRequestsOpen(!isRequestsOpen)}
                            className="relative hover:text-purple-400 transition-colors hover:scale-110 active:scale-95 transform"
                            title="طلبات الصداقة"
                        >
                            <User size={20} /> {/* Using User icon as base, maybe UserPlus if available */}
                        ...
                        <AnimatePresence>
                            {isRequestsOpen && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0, y: 10 }}
                                    className="absolute left-1/2 -translate-x-1/2 top-full mt-4 w-72 bg-[#151515] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-[60]"
                                >
                                    <div className="p-3 border-b border-white/10 flex justify-between items-center">
                                        <h3 className="font-bold text-sm">طلبات الصداقة</h3>
                                        <span className="text-xs text-gray-400">{notifications.requests} طلبات جديدة</span>
                                    </div>
                                    <div className="max-h-64 overflow-y-auto">
                                        {pendingRequestsList.length === 0 ? (
                                            <div className="p-4 text-center text-gray-500 text-sm">لا توجد طلبات جديدة</div>
                                        ) : (
                    ...
                    <button onClick={() => navigate('/submit')} className="hover:text-purple-400 transition-colors hover:scale-110 active:scale-95 transform" title="نشر كتاب"><PenTool size={20} /></button>

                    <button onClick={() => navigate('/chat')} className="relative hover:text-purple-400 transition-colors hover:scale-110 active:scale-95 transform" title="المحادثات">
                    ...
                    {user?.role === 'admin' && (
                        <button onClick={() => navigate('/admin')} className="relative hover:text-red-400 transition-colors hover:scale-110 active:scale-95 transform" title="لوحة التحكم">
                            <ShieldCheck size={20} />
